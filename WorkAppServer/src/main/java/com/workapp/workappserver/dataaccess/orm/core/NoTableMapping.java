...
public class NoTableMapping extends Mapping
{
	...
	public NoTableMapping(Class objectClass, NameGuesser nameGuesser)
	{
		...
		final Map<String, com.workapp.workappserver.dataaccess.orm.annotations.Column> annotationsMap = fieldsMaps[0];
		...
		for (String fieldName : gettersMap.keySet())
		{
			...
			com.workapp.workappserver.dataaccess.orm.annotations.Column annotation = annotationsMap.get(fieldName);

			// autoGenerated is not supported on @NoTable mappings
			if (annotation != null)
			{
				if (annotation.autoGenerated() == true) { throw new PersistException("@Column(autoGenerated=true) is set for field [" + fieldName + "] of class [" + objectClass.getCanonicalName() + " which has been declared with @NoTable"); }
			}
			...
			if (annotation != null && annotation.name() != null)
			{
				...
				if (annotation.name().trim().equals("")) { throw new PersistException("@Column annotation for field [" + fieldName + "] of class [" + objectClass.getCanonicalName() + "] defines a blank column name"); }
	...
	public String getFieldNameForColumn(String columnName)
	{
		...
		if (fieldName == null) { throw new PersistException("Could map field for column [" + columnName + "] on class [" + objectClass.getCanonicalName() + "]. Please specify an explict @Column annotation for that column."); }
		return fieldName;
	}
	...
	private void checkNameConflicts(String fieldName, String column)
	{
		...
		if (existingFieldName != null) { throw new PersistException("Fields [" + fieldName + "] and [" + existingFieldName + "] have conflicting column name [" + column + "] either from guessed names or anotations. " + "Please specify @Column mappings for at least one of those fields"); }
	}
	...
	private void checkAnnotation(Class objectClass)
	{
		...
		if (noTableAnnotation == null) { throw new PersistException("Class [" + objectClass.getCanonicalName() + "] does not specify a @NoTable annotation therefore it can't be mapped through NoTableMapping"); }
		...
		if (tableAnnotation != null) { throw new PersistException("Class [" + objectClass.getCanonicalName() + "] specifies conflicting @Table and @NoTable annotations"); }
	}
